...
UndirectedEdgeElements::UndirectedEdgeElements(
    sequence::Element *_forward_edge, sequence::Element *_backward_edge)
...
DynamicForest::DynamicForest(uint32_t num_vertices)
  ...
  edge_elements_ = std::vector<Element>{static_cast<std::size_t>(max_num_edges),
...
DynamicForest::DynamicForest(DynamicForest &&other) noexcept
    : num_vertices_{other.num_vertices_}, vertices_{std::move(other.vertices_)},
...
DynamicForest::AllocateEdgeElements(const UndirectedEdge &edge) {
...
void DynamicForest::DeleteEdge(const UndirectedEdge &edge) {
  ...
  // back together.
  const bool is_uv_before_vu_in_tour{uv->GetRepresentative() !=
                                     vu->GetRepresentative()};
